...
const App: React.FC = () => {
  ...
  const [metrics, setMetrics] = useState({
    totalRequests: 0,
    averageFulfillmentTimeSec: 0,
    averageWaitTimeSec: 0,
    averageTravelTimeSec: 0,
  });
  const [numFloors, setNumFloors] = useState(0);
  const [speed, setSpeed] = useState(1);

  // Fetch current elevator state from backend
  ...
  // Fetch elevator performance metrics
  ...
  // Poll simulation state and metrics every second
  useEffect(() => {
    fetchState();
    fetchMetrics();
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      ...
      {/* Main display area for elevators */}
      <div className="flex-1 overflow-auto p-6">
        <h1 className="text-2xl font-bold mb-6 text-gray-800">
          Elevator Simulation
        </h1>

        {/* Render each elevator column */}
